...
func indicate ():
	...
	if cooldownRemaining <= 0 && !Vars.objects[characterNode].anySkillCasting() && !indicating:
		...
		indicating = true
		effect = preload("res://Prefabs/Effects/SeedEffect.tscn").instance()
		arrowEffect = preload("res://Prefabs/Effects/ArrowEffect.tscn").instance()
		effect.drawArea = true
		effect.areaOfEffect = area
...
func castEnd(castAlready):
	if Client.selfPeerID == Vars.roomMaster && !castAlready:
		Vars.objects[characterNode].get_tree().root.get_node("Main").rpc_id(1,"objectCreated",Client.selfPeerID,"res://Prefabs/Objects/Seed.tscn",{"whoSummoned": characterNode, "position": Vars.objects[characterNode].position, "endPosition": castLocation, "area": area})
		casting = false
	...
	if Client.selfPeerID == characterNode:
		if !casting:
			if cooldownRemaining > 0:
				Vars.objects[characterNode].get_tree().root.get_node("Main/CanvasLayer/Skills/Skill1/Progress").get_material().set_shader_param("value",((cooldown - cooldownRemaining) / cooldown) * 100)
				Vars.objects[characterNode].get_tree().root.get_node("Main/CanvasLayer/Skills/Skill1/Progress").modulate = Color.red
			else:
				Vars.objects[characterNode].get_tree().root.get_node("Main/CanvasLayer/Skills/Skill1/Progress").get_material().set_shader_param("value",101)
				Vars.objects[characterNode].get_tree().root.get_node("Main/CanvasLayer/Skills/Skill1/Progress").modulate = Color.lime
		if indicating:
			effect.position = Vars.objects[characterNode].get_global_mouse_position()
			arrowEffect.points = PoolVector2Array([Vars.objects[characterNode].position,Vars.objects[characterNode].get_global_mouse_position()])
